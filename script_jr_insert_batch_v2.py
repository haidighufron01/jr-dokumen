...
table_name = '3_test_sample'
table_no_mapping_plat = '3_no_mapping_plat_test'
table_no_mapping_rangka = '3_no_mapping_rangka_test'
table_no_mapping_koding = '3_no_mapping_koding_test'
...
# df_processed, no_mapping = process_data(df)
...
# df_processed, no_mapping = process_data(df,no_rangka)
...
# df_processed= process_data(df,pkb_result)
...
# df_processed= process_data(pkb)


# df_processed= process_data(progresif)
...
# Example query: select all data from a table
query = f'SELECT * FROM data_papua'
# Execute the query and fetch the result into a Pandas DataFrame
result_data = client.execute(query)

column_names_query = f"DESCRIBE TABLE data_papua"
column_names_result = client.execute(column_names_query)
column_names = [desc[0] for desc in column_names_result]

# df = pl.DataFrame(result_data, columns=[desc[0] for desc in client.execute(f'DESCRIBE data_sample')])
df = pl.DataFrame(result_data, schema=[desc[0] for desc in column_names_result])
...
while not x.is_empty():
    subset = x.slice(i,step)
    if subset.is_empty():
        break  # Keluar dari loop jika subset kosong
    # Lakukan proses yang Anda inginkan pada subset data di sini

    batch_data = subset.to_pandas()
    # print(subset)
    # PROSES DATA
    # ===========================================================
    # Lakukan pemrosesan pada batch_data
    batch_data['no_rangka'] = batch_data['no_rangka'].str.slice(0, 8)
    batch_data['no_rangka'] = batch_data['no_rangka'].str.replace('*', '')
    batch_data = batch_data.applymap(clean_whitespace)
    df, no_mapping_plat = transform_data(batch_data)
